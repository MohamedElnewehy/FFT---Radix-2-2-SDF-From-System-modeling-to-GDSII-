module BF_1 #(
    parameter integer WIDTH_IN = 22, // Total input(real+imag)
    parameter integer FRAC_IN  = 10, // Q1.10
    parameter integer WIDTH    = 24, // Total output
    parameter integer FRAC     = 10  // Q2.10
)(
    input  wire [WIDTH_IN-1:0] data_in,
    input  wire [WIDTH-1:0]    data_in_from_reg, // same as output becouse it comes from the final destination [shift reg]
    input  wire                sel,
    ...
    wire signed [HW_IN-1:0] din_real = data_in[WIDTH_IN-1:HW_IN]; // real part is the most significant one
    wire signed [HW_IN-1:0] din_imag = data_in[HW_IN-1:0];        // imag part is the most significant one

    wire signed [HW-1:0] data_in_real_q;
    wire signed [HW-1:0] data_in_imag_q;

    // generic design for any future change in input/output sizing
    // is used to allign fraction parts
    generate
        if (SHIFT > 0) begin : g_shrink
        // input fraction part is bigger
            ...
            // Add 1 << (SHIFT - 1) (equivalent to +0.5) before right shift 
    ...
    // Sign-extend operands by 1 bit (to HW+1 bits) to prevent intermediate overflow
    ...
    wire signed [HW:0] ext_in_real     = {data_in_real_q[HW-1], data_in_real_q};
    wire signed [HW:0] ext_in_complex  = {data_in_imag_q[HW-1], data_in_imag_q};

    // Add/Subtract in HW+1 bits, then trancate
    wire signed [HW-1:0] plus_real     = (ext_reg_real + ext_in_real) ;
    wire signed [HW-1:0] plus_complex  = (ext_reg_complex + ext_in_complex) ;
    wire signed [HW-1:0] minus_real    = (ext_reg_real - ext_in_real) ;
    wire signed [HW-1:0] minus_complex = (ext_reg_complex - ext_in_complex) ;
